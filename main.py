...
def main():
    ...
    args.store_name = '_'.join(['STME_', args.dataset, args.modality, full_arch_name, args.consensus_type, 'segment%d' % args.num_segments, 'e{}'.format(args.epochs)])
    ...
    for group in policies:
        logger.info(
            ('[STME-{}]group: {} has {} params, lr_mult: {}, decay_mult: {}'.
...
def train(train_loader, model, criterion, optimizer, epoch, logger=None, scheduler=None):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        if i % args.print_freq == 0:
            logger.info(('Epoch: [{0}][{1}/{2}], lr: {lr:.8f}\t'
